var Crypt = function() {
	...
	this.PublicCurve = function(curve,genoratorPoint,order) {
		...
		this.sign = function(message,sk) {
			...
			if (this.adressToPublicKey(r)[1] != kG[1]) {
		...
		this.encryptWithPublicKey = function(Adress) {
			var rand = self.bufferToBigInt(self.randomBytes(this.OrderBytes))%this.n;
			var PublicKey = this.adressToPublicKey(Adress);
			var p = this.curve.multiply(PublicKey,rand);
			var SharedSecret = this.toAdress(p);
			var SharedKey = this.Hash(self.bigIntToBuffer(BigInt(SharedSecret)));
			var Public = this.toAdress(this.curve.multiply(this.g,rand));
			return {
				PublicKey:Public,
				SharedKey:SharedKey
			};
		}
		this.decryptWithPrivateKey = function(PublicVal,PrivateKey) {
			var SharedSecret = this.toAdress(this.curve.multiply(this.adressToPublicKey(PublicVal),PrivateKey));
			var SharedKey = this.Hash(self.bigIntToBuffer(BigInt(SharedSecret)));
			return SharedKey;
		}
	}
